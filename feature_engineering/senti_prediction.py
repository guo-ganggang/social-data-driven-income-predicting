...
#             for line in infile:
#                 temp = line.strip().split("\t")
#                 #income_level_flag = temp[1]
#                 uid = temp[0]
#                 text_seg = temp[1].strip().split(" ")
#                 income_level_flag = temp[2]
#                 words_vec_initi = []
#                 for i in range(400):
...
#                 output_file.write(uid + ',' + text_seg + ',' + text_vec + ',' + income_level_flag + '\n')
...
def weibo_senti_label(model_train_data_path,predict_data_path, features,target,result_output_path):
    ...
    classifier = lg_model.classify(predict_data)
...
if __name__ == "__main__":
    ...
    # ipath_2 = fpath + "train_data_20\\train_data_separate\\uid_text_incomeLevel.csv"
    # opath = fpath + "train_data_lda_senti\\uid_text_incomeLevelFlag_w2v.csv"
    # outFile_head = 'uid' + ',' + 'text_seg' + ',' + 'w2v_vec' + ',' + 'income_level_flag' + '\n'
    #
    # build_w2v_Vector(ipath_1, ipath_2, opath,outFile_head)

    # 构建LDA特征
    # filePath = "D:\\incomeLevelPrediction\\db_file\\"
    # modelFilePath = filePath + "LDA_model_clean_Kwords\\all_weibo_3_LDA_model"
    # infilePath = filePath + "train_data_lda_senti\\uid_text_incomeLevelFlag_w2v.csv"
    # predict_field_output_file = filePath + "train_data_lda_senti\\uid_text_incomeLevelFlag_lda_w2v.csv"
    # filePath = ""
    # modelFilePath = filePath + "all_weibo_3_LDA_model"
    # infilePath = filePath + "uid_text_incomeLevelFlag_w2v.csv"
    # predict_data_field = "text_seg"
    # predict_field_output_file = filePath + "uid_text_incomeLevelFlag_lda_w2v.csv"
    ...
    # model_train_data_path = file_path + "senti_6class\\word2vec_ldaFeature\\senti_train_data_word2vec-2-400_lda_feature_4class_v3.csv"
    # predict_data_path = file_path + "temp_baoqiang\\baoqiang_event_data_seg_clean_w2v_lda.csv"
    # result_output_path = file_path + "temp_baoqiang\\baoqiang_event_data_seg_clean_w2v_lda_senti_4class_v3_bt_200.csv"

    file_path = ""
    model_train_data_path = file_path + "senti_train_data_word2vec-2-400_lda_feature_4class_v4.csv"
    predict_data_path = file_path + "uid_text_incomeLevelFlag_lda_w2v.csv"
    result_output_path = file_path + "uid_textSeg_w2v_lda_sentiFlag_incomeLevelFlag.csv"
    #
